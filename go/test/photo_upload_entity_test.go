...
func TestPhotoUploadEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_PHOTO_UPLOAD_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		photoUploadRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath([]any{"new", "photo_upload"}, setup.data), "photo_upload_ref01"))

		photoUploadRef01DataResult, err := photoUploadRef01Ent.Create(photoUploadRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		photoUploadRef01Data = core.ToMapAny(photoUploadRef01DataResult)
...
func photo_uploadBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RAILWAYSTATIONPHOTOS_TEST_PHOTO_UPLOAD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RAILWAYSTATIONPHOTOS_TEST_PHOTO_UPLOAD_ENTID"])
