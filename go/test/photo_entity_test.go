...
func TestPhotoEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func photoBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID")
	...
	idmapResolved := core.ToMapAny(env["RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID"])
