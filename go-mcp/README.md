...
MCP server exposing the RailwayStationPhotos SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `railway-station-photos_list` | `entity`, optional `query` map | First page of records as JSON |
| `railway-station-photos_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
```sh
go build -o railway-station-photos-mcp ./...
```
...
After install, restart Claude Code; the `railway-station-photos_list` and
`railway-station-photos_load` tools become available in new sessions.
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"railway-station-photos_load","arguments":{"entity":"admin_inbox","query":{"id":1}}}}'
```
