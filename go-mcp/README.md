# railway-station-photos-mcp

[MCP](https://modelcontextprotocol.io) server exposing the RailwayStationPhotos SDK as
two agent tools — `railway-station-photos_list` and `railway-station-photos_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/railway-station-photos-mcp)
make build

# 2. Provide credentials via the environment
export RAILWAY_STATION_PHOTOS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user railway-station-photos \
  -- /absolute/path/to/railway-station-photos-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./railway-station-photos-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// railway-station-photos_list: first page of records
{ "entity": "country" }
{ "entity": "country", "query": { } }

// railway-station-photos_load: one record by id
{ "entity": "inbox_count", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/railway-station-photos-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user railway-station-photos \
     -- "$PWD"/dist/*/railway-station-photos-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `railway-station-photos_list` and `railway-station-photos_load` tools now appear
   in new sessions. Ask the agent to *"list country using railway-station-photos"*
   and it calls `railway-station-photos_list` with `{"entity":"country"}`.
...
```sh
export RAILWAY_STATION_PHOTOS_APIKEY=sk_live_xxx            # API key
export RAILWAY_STATION_PHOTOS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `railway-station-photos_list` tool
...
### Call the `railway-station-photos_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `railway-station-photos_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `railway-station-photos_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RAILWAY_STATION_PHOTOS_APIKEY` | API key sent with every request. |
| `RAILWAY_STATION_PHOTOS_BASE` | Optional override of the API base URL. |
...
```sh
./railway-station-photos-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"railway-station-photos_load","arguments":{"entity":"inbox_count","query":{"id":1}}}}'
```
