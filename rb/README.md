...
The Ruby SDK for the RailwayStationPhotos API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.AdminInbox(nil).create({ "name" => "Example" }, nil)
...
result, err = client.RailwayStationPhotos(nil).load(
  { "id" => "test01" }, nil
)
