...
class PublicInboxDirectTest < Minitest::Test
  def test_direct_list_public_inbox
    setup = public_inbox_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-public_inbox", setup[:live] ? "live" : "unit")
    if _should_skip
...
def public_inbox_direct_setup(mockres)
