...
class PhotoStationDirectTest < Minitest::Test
  def test_direct_list_photo_station
    setup = photo_station_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-photo_station", setup[:live] ? "live" : "unit")
    if _should_skip
  ...
  def test_direct_load_photo_station
    setup = photo_station_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-photo_station", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
...
def photo_station_direct_setup(mockres)
