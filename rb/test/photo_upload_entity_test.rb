...
class PhotoUploadEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = photo_upload_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "photo_upload." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    photo_upload_ref01_ent = client.PhotoUpload(nil)
...
def photo_upload_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "photo_upload", "PhotoUploadTestData.json")
