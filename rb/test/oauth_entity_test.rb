...
class OauthEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = oauth_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "oauth." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_OAUTH_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    oauth_ref01_ent = client.Oauth(nil)
    oauth_ref01_data = Helpers.to_map(Vs.getprop(
      Vs.getpath(setup[:data], "new.oauth"), "oauth_ref01"))

    oauth_ref01_data_result, err = oauth_ref01_ent.create(oauth_ref01_data, nil)
    assert_nil err
    oauth_ref01_data = Helpers.to_map(oauth_ref01_data_result)
    assert !oauth_ref01_data.nil?

    # LOAD
    oauth_ref01_match_dt0 = {}
    oauth_ref01_data_dt0_loaded, err = oauth_ref01_ent.load(oauth_ref01_match_dt0, nil)
    assert_nil err
    assert !oauth_ref01_data_dt0_loaded.nil?

  end
end

def oauth_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "oauth", "OauthTestData.json")
  ...
  entid_env_raw = ENV["RAILWAYSTATIONPHOTOS_TEST_OAUTH_ENTID"]
