...
class PhotographerDirectTest < Minitest::Test
  def test_direct_load_photographer
    setup = photographer_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-photographer", setup[:live] ? "live" : "unit")
    if _should_skip
...
def photographer_direct_setup(mockres)
