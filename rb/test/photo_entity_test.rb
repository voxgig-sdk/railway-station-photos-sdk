...
class PhotoEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    photo_ref01_data = nil
    if photo_ref01_data_raw.length > 0
    ...
    assert !photo_ref01_data_dt0_loaded.nil?
...
def photo_basic_setup(extra)
  ...
  entid_env_raw = ENV["RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID"]
