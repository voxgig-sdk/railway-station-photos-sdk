...
class PhotoDownloadEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = photo_download_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "photo_download." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_PHOTO_DOWNLOAD_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    photo_download_ref01_data = nil
    if photo_download_ref01_data_raw.length > 0
      photo_download_ref01_data = Helpers.to_map(photo_download_ref01_data_raw[0][1])
    end

    # LOAD
    photo_download_ref01_ent = client.PhotoDownload(nil)
    photo_download_ref01_match_dt0 = {}
    photo_download_ref01_data_dt0_loaded, err = photo_download_ref01_ent.load(photo_download_ref01_match_dt0, nil)
    assert_nil err
    assert !photo_download_ref01_data_dt0_loaded.nil?

  end
end

def photo_download_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "photo_download", "PhotoDownloadTestData.json")
  ...
  entid_env_raw = ENV["RAILWAYSTATIONPHOTOS_TEST_PHOTO_DOWNLOAD_ENTID"]
