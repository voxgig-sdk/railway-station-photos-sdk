...
class InboxEntryDirectTest < Minitest::Test
  def test_direct_list_inbox_entry
    setup = inbox_entry_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-inbox_entry", setup[:live] ? "live" : "unit")
    if _should_skip
...
def inbox_entry_direct_setup(mockres)
