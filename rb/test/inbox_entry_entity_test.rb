...
class InboxEntryEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.InboxEntry(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.InboxEntry(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    inbox_entry_ref01_list_result = inbox_entry_ref01_ent.list(inbox_entry_ref01_match, nil)
