...
class InboxCountEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = inbox_count_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "inbox_count." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_INBOX_COUNT_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    inbox_count_ref01_data = nil
    if inbox_count_ref01_data_raw.length > 0
...
def inbox_count_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "inbox_count", "InboxCountTestData.json")
  ...
  entid_env_raw = ENV["RAILWAYSTATIONPHOTOS_TEST_INBOX_COUNT_ENTID"]
