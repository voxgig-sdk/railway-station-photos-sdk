...
module RailwayStationPhotosUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RailwayStationPhotosResponse.new({})
    ...
    if fetch_err
      ...
      response = RailwayStationPhotosResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RailwayStationPhotosResponse.new(fetched)
    else
