...
class PhotoStationEntity
  ...
  def make
    opts = @_entopts.dup
    PhotoStationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = RailwayStationPhotosHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [PhotoStation, Hash] the current PhotoStation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = RailwayStationPhotosHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of PhotoStation fields)
  def match_get
  ...
  #
  # @param reqmatch [PhotoStationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.PhotoStation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [PhotoStation, Hash] the loaded PhotoStation; raises RailwayStationPhotosError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RailwayStationPhotosHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [PhotoStationListMatch, Hash, nil] match filter (any subset of
  #   PhotoStation fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<PhotoStation>, Array] the matching PhotoStation items; raises RailwayStationPhotosError on failure
