...
describe('StatEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set RAILWAY_STATION_PHOTOS_TEST_STAT_ENTID JSON to run live')
      return
    }
    ...
    const stat_ref01_data_dt0 = await stat_ref01_ent.load(stat_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['RAILWAY_STATION_PHOTOS_TEST_STAT_ENTID']
