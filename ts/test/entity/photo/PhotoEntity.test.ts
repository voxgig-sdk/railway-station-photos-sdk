...
describe('PhotoEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set RAILWAY_STATION_PHOTOS_TEST_PHOTO_ENTID JSON to run live')
      return
    }
...
function basicSetup(extra?: any) {
  ...
  let idmap = transform(
    ['photo01','photo02','photo03','photo01','photo02','photo03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',
  ...
  const idmapEnvVal = process.env['RAILWAY_STATION_PHOTOS_TEST_PHOTO_ENTID']
