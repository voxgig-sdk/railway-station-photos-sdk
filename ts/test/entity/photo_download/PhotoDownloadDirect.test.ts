...
describe('PhotoDownloadDirect', async () => {
  ...
  test('direct-load-photo_download', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-photo_download', setup.live)) return
    if (skipIfMissingIds(t, setup, ["filename01"])) return
