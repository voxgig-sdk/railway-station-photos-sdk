...
// RailwayStationPhotos SDK feature test
//
...
class FtCtrl extends RailwayStationPhotosControl
{
...
class FtClient
{
    ...
    private RailwayStationPhotosUtility $utility;

    public function __construct(RailwayStationPhotosUtility $utility, array $options)
    {
    ...
    public function get_utility(): RailwayStationPhotosUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RailwayStationPhotosContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RailwayStationPhotosContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RailwayStationPhotosContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RailwayStationPhotosConfig::make_feature($name);
    ...
    private static function buildUrl(RailwayStationPhotosSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RailwayStationPhotosError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RailwayStationPhotosResponse($fetched);
        }
    ...
    private function fail(RailwayStationPhotosContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RailwayStationPhotosContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
