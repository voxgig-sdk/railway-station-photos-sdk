...
class PhotoDownloadEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set RAILWAYSTATIONPHOTOS_TEST_PHOTO_DOWNLOAD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function photo_download_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("RAILWAYSTATIONPHOTOS_TEST_PHOTO_DOWNLOAD_ENTID");
