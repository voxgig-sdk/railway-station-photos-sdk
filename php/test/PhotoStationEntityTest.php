...
class PhotoStationEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = RailwayStationPhotosSDK::test(null, null);
        $ent = $testsdk->PhotoStation(null);
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->PhotoStation(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->PhotoStation(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = photo_station_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "photo_station." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $photo_station_ref01_data = null;
        if (count($photo_station_ref01_data_raw) > 0) {
...
function photo_station_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/photo_station/PhotoStationTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["photo_station01", "photo_station02", "photo_station03", "photo_station_by_id01", "photo_station_by_id02", "photo_station_by_id03", "photo_stations_by_country01", "photo_stations_by_country02", "photo_stations_by_country03", "photo_stations_by_photographer01", "photo_stations_by_photographer02", "photo_stations_by_photographer03"] as $k) {
