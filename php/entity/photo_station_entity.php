...
// RailwayStationPhotos SDK PhotoStation entity
...
class PhotoStationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new PhotoStationEntity($this->_client, $opts);
    }

    /**
     * @param PhotoStation|array $args PhotoStation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = RailwayStationPhotosHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return PhotoStation|array The current PhotoStation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of PhotoStation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = RailwayStationPhotosHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of PhotoStation fields).
     */
    ...
    /**
     * Load a single PhotoStation.
     *
     * @param PhotoStationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed PhotoStationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PhotoStation|array The loaded PhotoStation as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RailwayStationPhotosHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List PhotoStation items matching the given filter.
     *
     * @param PhotoStationListMatch|array|null $reqmatch Match filter (any subset
     *   of PhotoStation fields) as an assoc-array; PhotoStationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return PhotoStation[]|array A list of PhotoStation items as assoc-arrays at
