...
AQL-driven command-line client **and** interactive REPL for the RailwayStationPhotos
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/railway-station-photos-cli)
make build
...
RAILWAY_STATION_PHOTOS_BASE=https://api.example.com ./railway-station-photos-cli :help

# 5. No arguments -> interactive REPL
./railway-station-photos-cli
railway-station-photos> :help
railway-station-photos> :quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/railway-station-photos-cli
   ```
...
3. **Run a query.** Evaluate an AQL expression against the API (or run with no
   ...
   ```sh
   ./dist/*/railway-station-photos-cli :help
   ```
...
./railway-station-photos-cli :help
```
...
Run with no arguments to open a REPL (prompt `railway-station-photos>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `admin_inbox`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/railway-station-photos-cli`. |
...
(`./railway-station-photos-cli <expr>`) and interactively (the REPL), and expressions compose the
