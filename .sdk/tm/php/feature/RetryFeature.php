...
// RailwayStationPhotos SDK retry feature
...
class RailwayStationPhotosRetryFeature extends RailwayStationPhotosBaseFeature
{
    ...
    public function init(RailwayStationPhotosContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RailwayStationPhotosContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RailwayStationPhotosContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RailwayStationPhotosContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
