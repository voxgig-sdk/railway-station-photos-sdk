...
The Lua SDK for the RailwayStationPhotos API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:AdminInbox(nil):create({ name = "Example" }, nil)
...
local result, err = client:RailwayStationPhotos(nil):load(
  { id = "test01" }, nil
)
