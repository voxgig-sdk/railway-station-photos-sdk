-- RailwayStationPhotos SDK AdminInbox entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local AdminInboxEntity = {}
AdminInboxEntity.__index = AdminInboxEntity


function AdminInboxEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, AdminInboxEntity)
  self._name = "admin_inbox"
  self._client = client
...
function AdminInboxEntity:get_name()
  return self._name
end


function AdminInboxEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return AdminInboxEntity.new(self._client, opts)
end


function AdminInboxEntity:data_set(args)
  if args ~= nil then
...
function AdminInboxEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function AdminInboxEntity:match_set(args)
  if args ~= nil then
...
function AdminInboxEntity:match_get()
...
function AdminInboxEntity:stream(action, args, callopts)
...
function AdminInboxEntity:_run_op(ctx, post_done)
  local utility = self._utility
