...
local PhotoDownloadEntity = {}
PhotoDownloadEntity.__index = PhotoDownloadEntity


function PhotoDownloadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PhotoDownloadEntity)
  self._name = "photo_download"
  self._client = client
...
function PhotoDownloadEntity:get_name()
  return self._name
end


function PhotoDownloadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PhotoDownloadEntity.new(self._client, opts)
end


function PhotoDownloadEntity:data_set(args)
  if args ~= nil then
...
function PhotoDownloadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PhotoDownloadEntity:match_set(args)
  if args ~= nil then
...
function PhotoDownloadEntity:match_get()
...
function PhotoDownloadEntity:stream(action, args, callopts)
...
function PhotoDownloadEntity:_run_op(ctx, post_done)
  local utility = self._utility
