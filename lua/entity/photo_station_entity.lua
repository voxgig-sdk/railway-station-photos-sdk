...
local PhotoStationEntity = {}
PhotoStationEntity.__index = PhotoStationEntity


function PhotoStationEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PhotoStationEntity)
  self._name = "photo_station"
  self._client = client
...
function PhotoStationEntity:get_name()
  return self._name
end


function PhotoStationEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PhotoStationEntity.new(self._client, opts)
end


function PhotoStationEntity:data_set(args)
  if args ~= nil then
...
function PhotoStationEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PhotoStationEntity:match_set(args)
  if args ~= nil then
...
function PhotoStationEntity:match_get()
...
function PhotoStationEntity:stream(action, args, callopts)
...
function PhotoStationEntity:_run_op(ctx, post_done)
  local utility = self._utility
