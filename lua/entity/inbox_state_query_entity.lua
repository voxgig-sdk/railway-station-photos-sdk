-- RailwayStationPhotos SDK InboxStateQuery entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local InboxStateQueryEntity = {}
InboxStateQueryEntity.__index = InboxStateQueryEntity


function InboxStateQueryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InboxStateQueryEntity)
  self._name = "inbox_state_query"
  self._client = client
...
function InboxStateQueryEntity:get_name()
  return self._name
end


function InboxStateQueryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InboxStateQueryEntity.new(self._client, opts)
end


function InboxStateQueryEntity:data_set(args)
  if args ~= nil then
...
function InboxStateQueryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InboxStateQueryEntity:match_set(args)
  if args ~= nil then
...
function InboxStateQueryEntity:match_get()
...
function InboxStateQueryEntity:stream(action, args, callopts)
...
function InboxStateQueryEntity:_run_op(ctx, post_done)
  local utility = self._utility
