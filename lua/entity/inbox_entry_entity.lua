...
local InboxEntryEntity = {}
InboxEntryEntity.__index = InboxEntryEntity


function InboxEntryEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InboxEntryEntity)
  self._name = "inbox_entry"
  self._client = client
...
function InboxEntryEntity:get_name()
  return self._name
end


function InboxEntryEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InboxEntryEntity.new(self._client, opts)
end


function InboxEntryEntity:data_set(args)
  if args ~= nil then
...
function InboxEntryEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InboxEntryEntity:match_set(args)
  if args ~= nil then
...
function InboxEntryEntity:match_get()
...
function InboxEntryEntity:stream(action, args, callopts)
...
function InboxEntryEntity:_run_op(ctx, post_done)
  local utility = self._utility
