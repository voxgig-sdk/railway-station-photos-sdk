...
local OAuthTokenEntity = {}
OAuthTokenEntity.__index = OAuthTokenEntity


function OAuthTokenEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OAuthTokenEntity)
  self._name = "o_auth_token"
  self._client = client
...
function OAuthTokenEntity:get_name()
  return self._name
end


function OAuthTokenEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OAuthTokenEntity.new(self._client, opts)
end


function OAuthTokenEntity:data_set(args)
  if args ~= nil then
...
function OAuthTokenEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OAuthTokenEntity:match_set(args)
  if args ~= nil then
...
function OAuthTokenEntity:match_get()
...
function OAuthTokenEntity:stream(action, args, callopts)
...
function OAuthTokenEntity:_run_op(ctx, post_done)
  local utility = self._utility
