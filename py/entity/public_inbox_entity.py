...
class PublicInboxEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicInboxEntity(self._client, opts)
    ...
    def match_get(self) -> PublicInbox:
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[PublicInbox]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.PublicInbox().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
