...
class PhotographerEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PhotographerEntity(self._client, opts)
    ...
    def data_get(self) -> Photographer:
    ...
    def match_get(self) -> Photographer:
        self._utility.feature_hook(self._entctx, "GetMatch")
        return vs.clone(self._match)

    
    def load(self, reqmatch=None, ctrl=None) -> Photographer:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Photographer().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
