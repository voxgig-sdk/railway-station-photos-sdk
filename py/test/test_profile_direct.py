...
class TestProfileDirect:
    ...
    def test_should_direct_load_profile(self):
        setup = _profile_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-profile", "live" if setup["live"] else "unit")
        if _skip:
