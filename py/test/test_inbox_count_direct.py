...
class TestInboxCountDirect:

    def test_should_direct_load_inbox_count(self):
        setup = _inbox_count_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-inbox_count", "live" if setup["live"] else "unit")
        if _skip:
...
def _inbox_count_direct_setup(mockres):
