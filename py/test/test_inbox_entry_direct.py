...
class TestInboxEntryDirect:

    def test_should_direct_list_inbox_entry(self):
        setup = _inbox_entry_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-inbox_entry", "live" if setup["live"] else "unit")
        if _skip:
...
def _inbox_entry_direct_setup(mockres):
