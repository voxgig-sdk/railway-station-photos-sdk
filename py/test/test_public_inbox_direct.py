...
class TestPublicInboxDirect:

    def test_should_direct_list_public_inbox(self):
        setup = _public_inbox_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-public_inbox", "live" if setup["live"] else "unit")
        if _skip:
...
def _public_inbox_direct_setup(mockres):
