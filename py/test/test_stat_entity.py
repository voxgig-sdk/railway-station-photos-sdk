    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RAILWAYSTATIONPHOTOS_TEST_STAT_ENTID JSON to run live")
        client = setup["client"]
        ...
        stat_ref01_data_dt0_loaded, err = stat_ref01_ent.load(stat_ref01_match_dt0, None)
        assert err is None
...
def _stat_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "RAILWAYSTATIONPHOTOS_TEST_STAT_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("RAILWAYSTATIONPHOTOS_TEST_STAT_ENTID"))
