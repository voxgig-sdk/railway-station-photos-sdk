...
class TestPhotoDownloadDirect:

    def test_should_direct_load_photo_download(self):
        setup = _photo_download_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-photo_download", "live" if setup["live"] else "unit")
        if _skip:
...
def _photo_download_direct_setup(mockres):
