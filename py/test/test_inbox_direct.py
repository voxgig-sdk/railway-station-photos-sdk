...
class TestInboxDirect:
    ...
    def test_should_direct_list_inbox(self):
        setup = _inbox_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-inbox", "live" if setup["live"] else "unit")
        if _skip:
