    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        photo_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.photo")))
        photo_ref01_data = None
        if len(photo_ref01_data_raw) > 0:
...
def _photo_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("RAILWAYSTATIONPHOTOS_TEST_PHOTO_ENTID"))
